...
null_distribution_prop <- promotions %>%
  specify(formula = decision ~ gender, success = "promoted") %>%
  hypothesize(null = "independence") %>%
  generate(reps = 1000, type = "permute") %>%
  calculate(stat = "diff in props", order = c("male", "female"))

null_distribution_chi_squared <- promotions %>%
  specify(decision ~ gender, success = "promoted") %>%
  hypothesize(null = "independence") %>%
  generate(reps = 1000, type = "permute") %>%
  calculate(stat = "Chisq", order = c("male", "female"))
null_distribution_chi_squared

visualize(null_distribution_chi_squared)

result <- chisq.test(table(promotions$decision, promotions$gender))
# Print the result
print(result)
